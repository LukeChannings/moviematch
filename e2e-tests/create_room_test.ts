...
import { browserTest, btnSelector, selectInputSelector, textInputSelector, autosuggestInputSelector, autosuggestValueSelector } from "./_utils.ts";
import { loginAnonymous } from "./login_test.ts";

browserTest("Create Room - No filters", async (page: Page, emulatedName?: string) => {
  ...
  await page.screenshot({ path: `screenshots/create_room_nofilters_${emulatedName ?? "desktop"}.jpeg` });
});

browserTest("Create Room - With filters", async (page: Page, emulatedName?: string) => {
  ...
  await page.screenshot({ path: `screenshots/create_room_filters_${emulatedName ?? "desktop"}.jpeg` });
